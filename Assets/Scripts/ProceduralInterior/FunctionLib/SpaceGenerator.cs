...
public static class SpaceGenerator
{
    public static void CalculateRoomSpaces(Interior interior, int roomIndex)
    {
        if (!interior)
        {
            Debug.LogError("null interior");
            return;
        }
        else if (roomIndex < 0)
        {
            Debug.LogError($"room index {roomIndex}");
            return;
        }
        else if (roomIndex >= interior.Rooms.Count)
        {
            Debug.LogError($"Room.Count {interior.Rooms.Count} >= {roomIndex}");
            return;
        }
        ...
        while (remaining > 0)
        {
            ...
            Debug.Assert(area > 0, "CalculateRoomSpaces() -> GetLargestRectangleFromGrid() returned false. This should not happen");
